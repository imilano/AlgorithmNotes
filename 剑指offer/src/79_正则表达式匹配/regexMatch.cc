...
/* 题解：
    ...
    不为‘*’：
        - pattern下一个字符不为‘*’：
            - 这种情况比较简单，直接匹配当前字符。如果匹配成功，继续匹配下一个；如果匹配失败，直接返回false。注意这里的“匹配成功”，除了两个字符相同的情况外，还有一种情况，就是pattern的
              当前字符为‘.’,同时str的当前字符不为‘\0’。
            - pattern下一个字符为‘*’时，稍微复杂一些，因为‘*’可以代表0个或多个。
              这里把这些情况都考虑到：
               - （1）当‘*’匹配0个字符时，str当前字符不变，pattern当前字符后移两位，跳过这个‘*’符号；
               - （2）当‘*’匹配1个字符时，str当前字符移向下一个，pattern向后移动两位。
               - （3）当'*‘匹配多个字符时，str当前字符移向下一个，pattern当前字符不变。
*/
