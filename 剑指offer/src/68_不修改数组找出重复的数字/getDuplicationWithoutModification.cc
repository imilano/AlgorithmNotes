// 题目描述：在一个长度为n+1的数组里的所有数字都在1~n的范围内，所以数组中至少有一个数字是重复的。请找出数组中任意一个重复的数字，
// 但不能修改输入的数组。

// 题解：把1～n的数字从中间开始分为两部分，前一部分为1～m，后一部分为m+1～n。扫描数组，如果1～m范围内的数字超过m，
//      那么重复字肯定位于该区间；否则，m+1～n范围内一定包含重复的数字。
