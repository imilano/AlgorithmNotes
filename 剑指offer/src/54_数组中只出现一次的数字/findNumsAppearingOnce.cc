...
// 题解：使用异或运算。一个数字跟自己本身异或，其值为0.于是我们把数组中的所有数字都进行异或，最后的结果就是两个只出现一次的数字相异或的结果。
// 在该结果中找到第一位值为1的数位，将数组中其他数字以该位是否为1为标准 ，将原数字分为两组。因为两个相同的数字各位都相同，所以 相同的数字都分到同一组，
// 而1是两个不同数字异或的结果，于是，两个不同的数字 肯定会被分到不同组。再把这两组的数字分别异或，就能分别找出这两个数字。
