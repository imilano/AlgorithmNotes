 ...
 * 题解：这是一道动态规划题。
 *      首先，扔n个骰子的解可以由n-1骰子的解推出，即原文提包含了子问题的解。其次，对于如果使用递归的话，那么必定存在重复的子问题，因此，本题适用于动态规划的解法。
 *      首先构造状态转移方程：设扔n个骰子时的排列数之和为s的函数为f(n，s)，那么如果先扔了n-1个骰子，则扔最后一个骰子就有6种情况（最后一个扔到1、2、3、4、5、6）来得到排列数之和s。
 *      即为f(n,s) = f(n-1,s-1)+f(n-1,s-2)+f(n-1,s-3)+f(n-1,s-4)+f(n-1,s-5)+f(n-1,s-6);当s>6n时，f(n,s)等于0。初始状态为 n=1，f(1,1)=f(1,2)=f(1,3)=f(1,4)=f(1,5)=f(1,6)=1。
 *   
...
class Solution{
        public:
                int GetNumberCount(int num, int sum) {
                  ...
                  if (num == 1) return 1; // 只有一个骰子的时候，得到s的次数都只为1
